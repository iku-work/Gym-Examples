...
class MeshDataProcessor():        
    ...
    def _construct_datagrid( self ): #FIXME: appropriate name
        ...
        for input_object in self.input_objects:
            ...
            for req in self.attribute_requests:
                ...
                if req.type == 'no':  columns[ req.ident ] = data_iterator(                         mesh.vertices, 'normal',v_indices )
                ...
                if req.type == 'vertex_float':
                    columns[ req.ident ] = custom_vertex_float_iterator( bm, req, v_indices )
                
            
            for req in self.uniform_requests:
                if req.type == 'tex':          columns[ req.ident ] = ( get_image_ptr( mesh.uv_textures[ req.source ].data[ p_index ].image ) for p_index in p_indices )
                if req.type == 'major_axis':   columns[ req.ident ] = (   get_major_axis( R * mesh.polygons[ p_index ].normal )               for p_index in p_indices )
                if req.type == '_mat_index':   columns[ req.ident ] = ( get_material_ptr( o, mesh.polygons[ p_index ] )                       for p_index in p_indices )
                if req.type == '_obj_index':   columns[ req.ident ] = ( input_object.obj_index                                                for _       in p_indices )
                if req.type == 'cubic_cell':   columns[ req.ident ] = ( get_cubic_cell( mesh.polygons[ p_index ].center )                     for p_index in p_indices )
            #    if req.type == 'closest_omni': columns[ req.ident ] = ( None                                                                  for p_index in p_indices )
            ...
            for field_name in loop_array.dtype.names:
                ...
                sys.stdout.flush()
                loop_array[ field_name ] = list( columns[ field_name ] )
            print()
            
            #printArray( 'all', loop_array )
    ...
    def run( self, objects, per_object_properties, export_source_setup, reference_matrix, use_shared_vertex_data=False ):
        attributes             = export_source_setup.attributes
        uniforms               = export_source_setup.groupby
        # Processor parameters =================
        self.reference_matrix = reference_matrix
        self.input_objects    = sorted([ InputObject(i, o, per_object_properties[i] ) for i, o in enumerate( objects ) ] )
        ...
        self.uniform_requests          = [ UniformRequest(     *u.split(':') ) for u in uniforms                            ]
        self.uniform_requests         += [ UniformRequest(     *u.split(':') ) for u in export_source_setup.mesh_textures   ]
        ...
        for req in self.uniform_requests:
            if req.type == '_mat_index':    self.uni_dtype.append(  ('_mat_index',      'i4', 1) )
            if req.type == 'tex':           self.uni_dtype.append(  ( req.ident, 'i4', 1) ); self.requested_uv_textures[req.source] = UVTextureRequest( req.source, req.role )
            if req.type == 'major_axis':    self.uni_dtype.append(  ( req.type, 'i4', 1) )
            if req.type == '_obj_index':    self.uni_dtype.append(  ( req.type, 'i4', 1) )
            if req.type == 'cubic_cell':    self.uni_dtype.append(  ( 'cubic_cell', 'i4', 3) )
